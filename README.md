...
日报选择控件沿用 React 页面统一视觉；钉钉文本按模板结果原样发送，不会自动添加 `@所有人`。当前 1.4.6 仍有字体大小和时间下拉默认显示细节待后续调整。

任务详情按基本信息、钉钉机器人、消息模板与数据源、运行记录顺序显示。模板与字段面板按 7:3 左右布局，字段按“完整数据页 → 原始数据库 → 数据字段”逐级选择；预览在模板下方向下展开，测试发送紧跟预览结果。应用窗口最小尺寸为 1100×700。

任务列表负责新建、启停和删除，并区分配置未完成、待测试、可启用、已启用和计划异常。任务必须停用后才能删除；从未安装 Windows 任务计划的新任务也可直接删除。发送时间自动保存，已启用任务修改时间会立即更新计划；修改模板、字段或凭据会自动停用并要求重新测试。运行记录默认显示最近 5 条并可查看最多 100 条。
