...
生产消息使用“录入消息 → 解析确认 → 完成”三步页面：解析后自动读取目标 Notion Schema 并检查同日期记录；核对字段以目标库映射为准，编辑值后只有实际发生变化才重新检查，冲突按字段选择保留原值或使用新值，写入前服务端再次复查。塔筒消息只把“当日”值写入日报库，当月和全年累计不回填日报或新增月、年记录。后续模块按当前正式 React 外壳、暖中性配色、squircle 控件和标准字重逐步迁移，不再以旧原生页面或旧绿色样式作为新设计指导。
