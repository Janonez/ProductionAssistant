...
export function ReportDatePicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return <div className="date-picker">
    ...
    {open && <><button type="button" className="date-backdrop" aria-label="关闭日期选择器" onClick={() => setOpen(false)} /><motion.div className="calendar-popover" role="dialog" aria-label="选择日期" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0, transitionEnd: { transform: "none" } }} transition={{ duration: .14 }}>
